...
PROGRAM_NAME = "CryptNote"  # Name for our app
root.title('{} - {}'.format("Untitled", PROGRAM_NAME))
...
def new_file(event=None): #A Tkinter application runs most of its time inside an event loop, which is entered via the mainloop method. It waiting for events to happen.
    ...
    root.title('{} - {}'.format("Untitled", PROGRAM_NAME))  # default name of a new file
    global file_name
    file_name = None
...
def open_file(event=None):
    ...
    if input_file_name:
        ...
        with open(file_name,"r",encoding="utf-8") as file:
            file.seek(0)
            cryptname = file.name + "_crypt"
            if os.path.exists(cryptname):
                with open(cryptname,"ab+") as crypt:
                    crypt.seek(0)
                    key = int(pickle.load(crypt))
                decryptedfile = cipher.decrypt(file.read(),key)
                content_text.insert(1.0,decryptedfile) # function to read the decrypted file requested
            else:
                content_text.insert(1.0,file.read()) # function to read the file requested
...
def save_as(event=None):
    input_file_name = tkinter.filedialog.asksaveasfilename(initialfile='Untitled.txt', defaultextension=".txt", filetypes=[("All Files", "*.*"),
                                                                                               ("Text Docs", "*.txt"),
                                                                                               ("HTML", "*.html"),
                                                                                               ("CSS", "*.css"),
                                                                                               ("JavaScript", "*.js"),
    ...
    if input_file_name:
        ...
        _encrypt = tkinter.messagebox.askyesnocancel(PROGRAM_NAME,'Do you want to encrypt the file?')
        if _encrypt:
            key = random.randint(1,20)
            file = open(file_name,"w+",encoding="utf-8")
            file.seek(0)
            encryptedfile = cipher.encrypt(content_text.get(1.0,END),key)
            file.write(encryptedfile)
            cryptname = file.name+"_crypt"
            with open(cryptname,"wb") as crypt:
                crypt.seek(0)
                pickle.dump(str(key),crypt)
            file.close()
        elif _encrypt is not None:
...
def display_about(event=None):
    tkinter.messagebox.showinfo(
        "About", PROGRAM_NAME + "\n\nA basic text editor which puts security first.")


def display_help(event=None):
    tkinter.messagebox.showinfo(
        "Help", "This text editor works similar to any other editors apart from an option of encrypting your files.",
        icon='question')
...
save_as_icon = PhotoImage(file='icons/save_as.gif')
...
file_menu.add_command(label="Save As", accelerator='Ctrl+S', compound='left', image=save_as_icon, underline=0, command=save_as)
...
icons=('new_file', 'open_file', 'save_as', 'cut', 'copy', 'paste','undo', 'redo', 'find_text')
...
content_text.bind('<Control-o>', open_file)
content_text.bind('<Control-S>', save_as)
content_text.bind('<Control-s>', save_as)
